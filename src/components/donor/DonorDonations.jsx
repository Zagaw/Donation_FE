...
const DonorDonations = () => {
  ...
  const handleRequestExecution = async (matchId) => {
    if (!window.confirm('Have you delivered the donation? This will notify the admin to mark this match as executed.')) {
      return;
    }

    try {
      setActionLoading(true);
      await donationApi.requestExecution(matchId);
      alert('Request sent to admin successfully!');
      // Refresh matches to show the requested state
      const matchesRes = await donationApi.getMyMatches();
      setMatches(matchesRes.data.matches || []);
    } catch (error) {
